...
## Acceptance Criteria
- [ ] Given a user opens the app for the first time, when it launches, then
  `signInAnonymously()` is called automatically and completes with no user-visible
  login/signup UI.
- [ ] Given anonymous sign-in succeeds, when it's the very first sign-in for this
  install, then the `users/{uid}` document (ph-0-us-6) is created client-side with
  `createdAt` set.
- [ ] Given the user closes and reopens the app, when it launches again, then the existing
  Firebase Auth session is restored automatically — no new anonymous user is created, and no
  second `users/{uid}` document is written.
...
- [ ] Given a user is about to export their driving log (Phase 7) or has meaningfully
  ...
  exists so later phases can build the warning on top of it).
...
## Tasks
- [ ] Call `signInAnonymously()` on app launch if no existing session is restored.
- [ ] On first-ever successful sign-in, create the `users/{uid}` document (`createdAt`).
- [ ] Handle the offline-on-first-launch case with a dedicated no-connection UI state.
- [ ] Log the first-sign-in Analytics event.
- [ ] Expose the current `uid`/auth-ready state via app-wide state (context/store) so every
